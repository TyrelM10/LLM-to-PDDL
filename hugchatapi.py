...
def hugchatter(question):
    ...
    chatbot = hugchat.ChatBot(cookies=cookies.get_dict(), default_llm=1, system_prompt="You are an assistant to generate PDDL code for natural language text input descriptions and correct errors in PDDL code to solve in a STRIPS Planner. Instruction: Please do not add comments and explanations to the output and only give code in PDDL format in code blocks designed as ```pddl (Your Generated Code Here) ```. PLEASE DO NOT USE conditional expressions, functions or derived functions, equality or any syntax from higher than PDDL v1 but only in STRIPS implementation of PDDL.")  # or cookie_path="usercookies/<email>.json"
