...
def parse_name(iter, father):
    if not iter.peek().is_word():
        ...
        receiver({'error_number': 5, 'error':"There is a value error where {} predicate statement must contain a name!".format(father)})
...
def _parse_type_helper(iter, type_class):
    ...
    while not iter.empty():
        ...
        if type_class != Variable and len(var) > 0 and var[0] in reserved:
            ...
            receiver({'error_number': 6, 'error':"There is a value error where type must not begin with reserved char!"})
...
def parse_keyword(iter):
    ...
    if name == "":
        ...
        receiver({'error_number': 9, 'error':"There is a value error where empty keyword was found."})
...
def parse_parameters(iter):
    ...
    if not iter.try_match(":parameters"):
        ...
        # )
        receiver({'error_number': 13, 'error':'There is a value error where keyword ":parameters" is required before parameter list!'})
...
def _parse_types_with_error(iter, keyword, classt):
    if not iter.try_match(keyword):
        ...
        # )
        receiver({'error_number': 15, 'error':f'There is a value error where keyword "{keyword}" required before {classt.__name__}!'})
...
def _parse_domain_helper(iter, keyword):
    ...
    if not iter.try_match(keyword):
        ...
        # )
        receiver({'error_number': 16, 'error':"There is a value error where domain statement must be present before domain name!"})
...
def parse_formula(iter):
    ...
    if iter.is_structure():
        ...
        if key[0] in reserved:
            ...
            receiver({'error_number': 17, 'error':"There is a value error where formula must not start with reserved char!"})
...
def parse_predicates_stmt(iter):
    ...
    if not iter.try_match(":predicates"):
        ...
        # )
        receiver({'error_number': 20, 'error':"There is a value error where predicate definition must start with ':predicates' keyword."})
...
def parse_problem_name(iter):
    ...
    if not iter.try_match("problem"):
        ...
        # )
        receiver({'error_number': 24, 'error':"There is a value error where there is invalid problem name specification. Problem name definition must start with 'problem'."})
...
def parse_problem_def(iter):
    ...
    if not iter.try_match("define"):
        ...
        #     'must start with "define"'
        # )
        receiver({'error_number': 25, 'error':"There is a value error where there is invalid problem definition. Problem definition must start with 'define'."})
...
def parse_init_stmt(iter):
    ...
    if not iter.try_match(":init"):
        ...
        receiver({'error_number': 26, 'error':"There is a value error when found invalid keyword when parsing :init statement."})
...
def parse_goal_stmt(iter):
    ...
    if not iter.try_match(":goal"):
        ...
        receiver({'error_number': 27, 'error':"There is a value error on finding invalid keyword when parsing :goal statement."})
